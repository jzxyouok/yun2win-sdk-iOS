...
- (void)sendChannelId:(NSString *)channelId withMode:(VideoDataType)mediaType actionType:(VideoActionType)actionType toUids:(NSArray<NSString *> *)uids
{
    for (NSString *uid in uids) {
        [self.session.sessions getSessionWithTargetId:uid type:@"p2p" success:^(Y2WSession *session) {
            
            NSDictionary *messageDic = [VideoStatusManager sendChannelId:channelId sessionId:self.session.ID mediaType:mediaType videoActionType:actionType videoChatType:self.session.type formUid:self.session.sessions.user.ID toUids:uids];
            ...
            [self.session.sessions.user.bridge sendMessages:nil
                                                pushMessage:pns
                                                callMessage:messageDic
                                                  toSession:imSession];
        } failure:^(NSError *error) {
            NSLog(@"%@",error.message);
        }];
...
- (void)moreInputViewDidSelectItem:(MoreItem *)item {
    ...
    if ([item.title isEqualToString:@"语音通话"]) {
        [self openAVWithMode:VideoDataTypeAudio];
    }
    
    if ([item.title isEqualToString:@"视频通话"]) {
        [self openAVWithMode:VideoDataTypeVideo];
    }
...
- (void)messageCell:(MessageCell *)cell didClickBubbleView:(UIView *)view message:(Y2WBaseMessage *)message {
    ...
    if ([message.type isEqualToString:@"av"]) {
        if ([self.session.type isEqualToString:@"p2p"]) {
            [self openAVWithMode:VideoDataTypeVideo];
        }
    }
...
//        item = [[MoreItem alloc] init];
//        item.title = @"文档";
//        item.image = [UIImage y2w_imageNamed:@"输入框-文档"];
//        [items addObject:item];

        item = [[MoreItem alloc] init];
        item.title = @"语音通话";
        item.image = [UIImage y2w_imageNamed:@"输入框-语音通话"];
        [items addObject:item];
