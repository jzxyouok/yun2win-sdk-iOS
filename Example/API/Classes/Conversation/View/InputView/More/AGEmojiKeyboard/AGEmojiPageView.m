...
- (void)setButtonTexts:(NSMutableArray *)buttonTexts {
  ...
  if (([self.buttons count] - 1) == [buttonTexts count]) {
    ...
    for (NSUInteger i = 0; i < [buttonTexts count]; ++i) {
        NSDictionary *buttonDic = buttonTexts[i];
        UIImage *buttonImg = [UIImage imageWithContentsOfFile:buttonDic[@"path"]];
        NSString *buttonString = [NSString stringWithFormat:@"[%@]",buttonDic[@"name"]];
