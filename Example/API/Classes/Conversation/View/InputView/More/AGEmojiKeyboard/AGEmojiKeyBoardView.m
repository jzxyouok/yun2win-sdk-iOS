...
//      _emojis = [[NSDictionary dictionaryWithContentsOfFile:plistPath] copy];
    ...
    _emojis = [[NSDictionary dictionaryWithContentsOfFile:fileSavePath] copy];
  }
  return _emojis;
}
...
//    }
      [array addObject:[self.dataSource emojiKeyboardView:self imageForSelectedCategory:AGEmojiKeyboardViewCategoryImageFace]];
  });
  return array;
}
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource {
  self = [super initWithFrame:frame];
    if (self) {
        ...
        self.emojiSendBTN = [UIButton buttonWithType:UIButtonTypeCustom];
        self.emojiSendBTN.frame = CGRectMake(self.width - 60, self.height - 10, 50, 25);
        self.emojiSendBTN.layer.masksToBounds = YES;
        self.emojiSendBTN.layer.cornerRadius = 5;
        self.emojiSendBTN.layer.borderWidth = 1;
        self.emojiSendBTN.layer.borderColor = [UIColor colorWithHexString:@"E3EFEF"].CGColor;
        [self.emojiSendBTN setTitle:@"发送" forState:UIControlStateNormal];
        [self.emojiSendBTN setTitleColor:[UIColor colorWithHexString:@"E3EFEF"] forState:UIControlStateNormal];
        [self.emojiSendBTN addTarget:self action:@selector(emojiSend) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:self.emojiSendBTN];
...
- (void)emojiSend
{
    if ([self.delegate respondsToSelector:@selector(emojiKeyBoardViewDidPressSenderBtn:)]) {
        [self.delegate emojiKeyBoardViewDidPressSenderBtn:self];
    }
}
