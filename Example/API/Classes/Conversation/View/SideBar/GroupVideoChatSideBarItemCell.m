...
- (void)setItem:(GroupVideoChatSideBarItem *)item {
    ...
    [self.imageButton setImage:_item.highlightedImage forState:UIControlStateSelected];
...
- (void)onClickButton
{
    if ([self.delegate respondsToSelector:@selector(onTapItem:)]) {
        [self.delegate onTapItem:self.item];
    }
}
